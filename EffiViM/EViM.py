...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, stride=16, in_chans=3, embed_dim=1024, norm_layer=None, flatten=True):
        super().__init__()
        ...
        print(f"Feature Map Size：{self.img_size}, Patch Size：{patch_size},token numbers{self.num_patches}")
...
class EffiViM(nn.Module):
    def __init__(self):
        super().__init__()
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.vim_model = ViM().to(device)
        self.efficient_net = EfficientNet.from_pretrained("efficientnet-b0")
        blocks = self.efficient_net._blocks
        for i in range(len(blocks)):
            for param in blocks[i].parameters():
                param.requires_grad = i >= len(blocks) - 3
